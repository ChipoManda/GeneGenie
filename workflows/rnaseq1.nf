...
workflow RNASEQ {
    ...
    input_reads = Channel
        ...
        .map { row -> 
            def meta = [id: row.sample_id, single_end: params.read_type != 'paired']
            def reads = params.read_type == 'paired' ?
                [file(row.fastq_1), file(row.fastq_2)] :
                file(row.fastq)
            if (params.read_type == 'paired' && (reads[0] == null || reads[1] == null)) {
                error "Paired-end mode specified but FastQ file(s) missing for sample ${row.sample_id}"
            } else if (params.read_type == 'single' && reads == null) {
                error "Single-end mode specified but FastQ file missing for sample ${row.sample_id}"
            }
            [meta, reads]
        }
